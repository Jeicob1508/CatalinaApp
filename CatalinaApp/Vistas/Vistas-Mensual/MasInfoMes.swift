...
struct MasInfoMes: View {
    ...
    var body: some View {
        var formattedMonth: String {
            let monthString = globalState.fechaInfo / 10000
            
            let dateFormatter = DateFormatter()
            dateFormatter.dateFormat = "MM"
            
            if let month = dateFormatter.date(from: String(monthString)) {
            ...
            VStack{
                if !viewModel.listObject_BudgetD.isEmpty {
                    VStack{
                        ...
                        for i in 1...31 {
                            let temp = validarfecha(i: i)
                            if let object = viewModel.listObject_BudgetD.first(where: { $0.id == Int(temp) }) {
                                numEntrada += 1
                                vRMtratamiento += object.tratamiento
                                vRMpl_prod += object.PBProduccion
                                vRMpl_ley += object.PBCalidad
                                vRMpl_rec += object.PBRecuperacion
                                vRMpl_finos += object.PBFinos
                                vRMzn_prod += object.ZNProduccion
                                vRMzn_ley += object.ZNCalidad
                                vRMzn_rec += object.ZNRecuperacion
                                vRMzn_finos += object.ZNFinos
                            }
                        }
                    }
                    .onAppear {
    ...
    func validarfecha(i: Int) -> String {
        let fechaInfo = globalState.fechaInfo
        if String(fechaInfo).count == 5 {
            return String(i) + "0" + String(fechaInfo)
        } else {
            return String(i) + String(fechaInfo)
        }
    }
